...
type RuleConfig struct {
	...
	// DifficultyDropLowThreshold 是高难题占比相对降幅的 low 阈值（0~1）。
	DifficultyDropLowThreshold float64 `json:"difficulty_drop_low_threshold"`
	// DifficultyDropMediumThreshold 是高难题占比相对降幅的 medium 阈值（0~1）。
	DifficultyDropMediumThreshold float64 `json:"difficulty_drop_medium_threshold"`
	// DifficultyDropHighThreshold 是高难题占比相对降幅的 high 阈值（0~1）。
...
const (
	...
	// defaultDifficultyDropCurrentWindowDays: 高难占比当前窗口默认最近 7 天。
	defaultDifficultyDropCurrentWindowDays = 7
	// defaultDifficultyDropBaselineWindowDays: 高难占比历史基线默认前 30 天。
	defaultDifficultyDropBaselineWindowDays = 30
	// defaultDifficultyDropMinCurrentTotal: 当前窗口总题量低于 5 时，不做难度占比告警。
	defaultDifficultyDropMinCurrentTotal = 5
...
func (c RuleConfig) Validate() error {
	...
	if c.DifficultyDropCurrentWindowDays <= 0 {
		return fmt.Errorf("难度占比当前窗口天数必须大于 0")
	}
	if c.DifficultyDropBaselineWindowDays <= 0 {
		return fmt.Errorf("难度占比基线窗口天数必须大于 0")
	}
	if c.DifficultyDropMinCurrentTotal < 0 {
		return fmt.Errorf("难度占比检测当前窗口最小总题量必须大于或等于 0")
	}
